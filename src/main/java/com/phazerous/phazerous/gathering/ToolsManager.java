...
import com.phazerous.phazerous.models.PlayerVeinTool;
...
public class ToolsManager {
    ...
    public List<Pair<VeinTool, ItemStack>> getPlayerTools(Player player, List<Integer> toolsTypesIds) {
        Bson filter = new Document("uuid", player.getUniqueId());

        Document playerDoc = dbManager.getDocument(filter, CollectionType.PLAYER);
        PlayerModel playerModel = DocumentParser.parse(playerDoc, PlayerModel.class);

        List<ObjectId> veinToolsIds = playerModel.getTools().stream()
                .filter(veinTool -> toolsTypesIds.contains(veinTool.getToolType()))
                .map(PlayerVeinTool::getToolId)
                .collect(Collectors.toList());

        Bson veinToolsFilter = in("_id", veinToolsIds);
        List<Document> veinToolsDocs = dbManager.getDocuments(veinToolsFilter, CollectionType.TOOLS);
        List<Pair<VeinTool, ItemStack>> tools = veinToolsDocs.stream()
                .map(veinToolDoc -> {
                    VeinTool veinTool = DocumentParser.parse(veinToolDoc, VeinTool.class);
                    ItemStack toolItemStack = buildTool(veinTool);

                    return new Pair<>(veinTool, toolItemStack);
                })
                .collect(Collectors.toList());
    ...
    private ItemStack buildTool(VeinTool veinTool) {
        return new ItemBuilder(veinTool.getMaterial()).setDisplayName(veinTool.getTitle())
                .build();
    }
